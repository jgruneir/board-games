const boardGames = [
  ...
  { name: "Unfathomable", description: "A game of hidden traitors and maritime horror.", bggLink: "https://boardgamegeek.com/boardgame/340466/unfathomable", complexity: 8 },
...
function updateTogglePosition() {
  const topLeftBox = document.getElementById('topLeftBox');
  const toggleContainer = document.querySelector('.toggle-container');

  // Calculate the top and left positions of the toggle container
  const topLeftBoxRect = topLeftBox.getBoundingClientRect();
  const toggleContainerTop = topLeftBoxRect.top + window.scrollY + 'px' + 60;
  const toggleContainerLeft = topLeftBoxRect.left + window.scrollX + 'px';

  // Set the new position for the toggle container
  toggleContainer.style.top = toggleContainerTop;
  toggleContainer.style.left = toggleContainerLeft;
